...
const EditUserModal = ({ isOpen, onClose, user }) => {
  ...
  // Sync state with the selected user when modal opens
  useEffect(() => {
    if (user) {
      setFormData({
        name: user.name || "",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm transition-opacity">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 transform transition-all animate-in fade-in zoom-in duration-200">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Edit Member</h2>
            <p className="text-gray-500 text-xs mt-1 font-mono">{user._id}</p>
          </div>
          <button 
            onClick={onClose}
